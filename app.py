...
# Change Name & Logo
st.set_page_config(page_title="ROGSUCHAK", page_icon=r"C:\Users\shash\Downloads\DALL·E 2025-03-07 21.32.04 - A logo design with the text 'रोगसूचक' written in stylized Sanskrit typography. The design should have a modern medical theme with a caduceus (medical .webp")
...
st.markdown(
    ...
    body, p, label, div, span {
        color: lightblue !important;  /* Change text color to white */
    }
    </style>
    """,
...
background_image_url = "https://img.rasset.ie/0021f82c-1600.jpg"  # Replace with your image URL
...
models = {
    'diabetes': pickle.load(open(r'C:\Users\shash\OneDrive\vs code\internship\Project2_files\Medical diagnosis using AI\diabetes_model.sav', 'rb')),
    'heart_disease': pickle.load(open(r'C:\Users\shash\OneDrive\vs code\internship\Project2_files\Medical diagnosis using AI\heart_disease_model.sav', 'rb')),
    'parkinsons': pickle.load(open(r'C:\Users\shash\OneDrive\vs code\internship\Project2_files\Medical diagnosis using AI\parkinsons_model.sav', 'rb')),
    'lung_cancer': pickle.load(open(r'C:\Users\shash\OneDrive\vs code\internship\Project2_files\Medical diagnosis using AI\lungs_disease_model.sav', 'rb')),
    'thyroid': pickle.load(open(r'C:\Users\shash\OneDrive\vs code\internship\Project2_files\Medical diagnosis using AI\Thyroid_model.sav', 'rb')),
    'chronic_kidney_disease': pickle.load(open(r'C:\Users\shash\OneDrive\vs code\internship\Project2_files\Medical diagnosis using AI\chronic_kidney_disease.sav','rb'))
}
...
#Chronic_Kidney_Disease prediction page
if selected == "Chronic Kidney Disease Prediction":
    ...
    bp = display_input('BloodPressure', 'Enter the bp of the person', 'bp', 'number')
    sg = display_input('Specific Gravity', 'Enter the Specific Gravity of the person', 'sg', 'number')
    al = display_input('Albumin', 'Enter the Albumin of the person', 'al', 'number')
    su = display_input('Sugar', 'Enter the Sugar level of the person', 'su', 'number')
    bgr = display_input('Blood Glucose Random', 'Enter the Blood Glucose Random level of the person', 'bgr', 'number')
    bu = display_input('Blood Urea', 'Enter the Blood Urea level of the person', 'bu', 'number')
    sc = display_input('Serum Creatinine', 'Enter the Serum Creatinine level of the person', 'sc', 'number')
    sod = display_input('Sodium', 'Enter the Sodium level of the person', 'sod', 'number')
    pot = display_input('Potassium', 'Enter the Potassium level of the person', 'pot', 'number')
    hemo = display_input('Hemoglobin', 'Enter the Hemoglobin level of the person', 'hemo', 'number')
    pcv = display_input('Packed Cell Volume', 'Enter the Packed Cell Volume level of the person', 'pcv', 'number')
    wc = display_input('White Blood Cell Count', 'Enter the White Blood Cell Count of the person', 'wc', 'number')
    rc = display_input('Red Blood Cell Count', 'Enter the Red Blood Cell Count of the person', 'rc', 'number')
    chronic_kidney_disease_diagnosis =''
    if st.button("Chronic Kidney Disease Test Result"):
        chronic_kidney_disease_prediction = models['chronic_kidney_disease'].predict([[age, bp, sg, al, su, bgr, bu, sc, sod, pot, hemo, pcv, wc, rc]])
        chronic_kidney_disease_diagnosis = "The person has Chronic kidney disease" if chronic_kidney_disease_prediction[0] == 1 else "The person does not have Chronic Kidney disease"
        st.success(chronic_kidney_disease_diagnosis)    